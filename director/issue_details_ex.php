...
if (isset($_POST['data'])) {
    ...
    $select_issue = mysqli_query($conn, "SELECT DATE_FORMAT(`resource`.`expiration_time`, '%Y-%c-%d') as expiration_time, `resource`.`resource_name`, `resource`.`quantity`, `resource`.`update_time` 
                                         FROM `resource`                                                                    
                                         WHERE `resource`.`update_time` = '$date' ");
