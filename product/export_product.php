...
<?php
$result = mysqli_query($conn, "SELECT `products`.`product_name`, `products`.`quantity`,`products`.`product_id`,`employee`.`employee_id`, date_format(`products`.`expiration_date`, '%Y-%c-%e') as `expiration_date`,date_format(`products`.`created_date`, '%Y-%c-%e') as `created_date`, `products`.`status`
                                FROM `products`
                                ...
                                where `employee`.`employee_id` = '$id' and `products`.`status` = 'Đã lưu kho'
                                Order by `products`.`expiration_date` asc");

if (isset($_POST['require'])) {
    $product_id = $_POST['require'];
    $update_products = mysqli_query($conn, "UPDATE `products` set `status` = 'Yêu cầu xuất NVL' where `product_id` = $product_id");
                                                ...
                                                <tr class="action" style="text-align: center;">
                                                    ...
                                                    <td style="width: 13%; text-align: center;">
                                                        ...
                                                        <button type="submit" name="require" class="btn btn-info text-white"
                                                            data-toggle="modal"
                                                            value="<?php echo $row['product_id'] ?>">Yêu cầu</button>
                                                    </td>
                                                </tr>
                                                <?php
                                            }
