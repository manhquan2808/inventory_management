...
$result = mysqli_query($conn, "SELECT `products`.`product_name`, `products`.`quantity`, `products`.`status`, DATE_FORMAT(`products`.`created_date`, '%M %d %Y') as created_date, DATE_FORMAT(`products`.`expiration_date`, '%M %d %Y') as expiration_date,`shelves`.`shelves_name`
FROM `issue_products`
JOIN `shelves` on `shelves`.`shelves_id` = `issue_products`.`shelves_id`
JOIN `products_detail` on `products_detail`.`id` = `issue_products`.`products_detail_id`
...
WHERE `shelves`.`shelves_id` = $shelves_id and `products`.`status` = 'Đã lưu kho'
GROUP BY `products`.`product_id`, `inventory`.`inventory_id` 
