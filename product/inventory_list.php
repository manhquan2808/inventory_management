...
if (isset($_POST['submit'])) {
    ...
    if (isset($_POST['Inventory_name']) && count($check_box) > 0) {
        ...
        foreach ($check_box as $key => $value) {
            $sql = "UPDATE `products` SET status='Distributed' WHERE product_id=$value";
            ...
            if (mysqli_query($conn, $sql) && $update_inventory_status) {
                // Chèn issue_id và product_id vào bảng issue_productss
                // $query_insert_issue_products = mysqli_query($conn, "INSERT INTO `issue_productss`(`issue_id`, `product_id`, `status`) VALUES ('$issue_id', '$value', 'Available')");
            }
        }
                                            ...
                                            while ($row = mysqli_fetch_array($result)) {
                                                ...
                                                <tr class="action" style="text-align: center;">
                                                    ...
                                                    <td style="width: 13%; text-align: center;">
                                                        <a
                                                            href="./inventory_details.php?shelves_id=<?php echo $row['shelves_id'] ?>&product_manager_id=<?php echo $_SESSION['product_manager_id'] ?>">
                                                            Xem
                                                        </a>
                                                    </td>
                                                </tr>
