            ...
            <ul class="navbar-nav">
                ...
                <li class="nav-item dropdown">
                    <a class="nav-link dropdown-toggle text-muted waves-effect waves-dark" href="#" id="navbarDropdown"
                        ...
                        <?php
                        $sqlll = mysqli_query($conn, "SELECT * FROM `employee` Where `employee_id` = '$id'");
                        if (mysqli_num_rows($sqlll) > 0) {
                            $row = mysqli_fetch_assoc($sqlll);
                            ?>
                            <img class="rounded-circle" style="width: 40px; background: white;"
                                src="<?php echo $row['avt']; ?>" alt="khong co hinh">
                            <?php
                        }
                        ?>
